...
function Set-TeamsIcon {
    ...
    $Icon = [System.IO.File]::ReadAllBytes($iconfile)

    #getting teams
    $Apiuri = "https://graph.microsoft.com/v1.0/teams" + '?$filter=startsWith(displayName, ' + "'$TeamsName')"
    $team = (Invoke-RestMethod -Headers @{Authorization = "Bearer $($Token.access_token)" } -Uri $apiUri -Method Get).value

    $ApiUri = "https://graph.microsoft.com/v1.0/teams/$teamId/photo/$icon"

    Try {
        $out = Invoke-Restmethod -method PUT -uri $uri -Headers $header
    }
    catch {
        $out = $_
