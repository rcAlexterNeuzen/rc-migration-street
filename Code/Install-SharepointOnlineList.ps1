...
$Global:file = "." + $switch + "log" + $switch + "$(Get-Date -format "ddMMyyy-HHmm")-SharepointOnlineSetup.txt"
...
try {
    ...
    $Teams = RunQueryandEnumerateResults
...
ForEach ($Channel in $Sharepoint.Channels) {
    try {
        ...
        switch ($channel){
            "Fileshare Migrations" { $Sharepoint.ids.FileshareChannelid = $Channels.id}
            "Homefolder Migrations" { $Sharepoint.ids.HomeFolderChannelid = $Channels.id}
            "Errors" { $Sharepoint.ids.ErrorChannelid = $Channels.id}
        }
...
do {
    ...
    $sites = RunQueryandEnumerateResults 
    start-sleep -seconds 2
...
$Import = "$($Sharepoint.FileShare)" , "$($sharepoint.HomeDir)", "$($sharepoint.Errors)"
[array]$lists = @()
...
$siteId = ($sites | Where-Object {$_.Displayname -eq $Sharepoint.TeamsName}).weburl.split("/")[2] + ":/sites/" + ($sites | Where-Object {$_.Displayname -eq $Sharepoint.TeamsName}).weburl.split("/")[4] + ":/lists"

forEach ($list in $lists) { 
    try {
        $createlist = Create-List -DisplayName $list.name -SiteId $siteId 
        $list.id = $createList.id
        $list.WebUrl = $CreateList.webUrl
        Log-Message -file $file -status DONE -Message "$($List.name) list is created with url: $($List.WebUrl)"
    }
    catch {
        $connectionError = $_
        Log-message -File $file -Status Error -Message "Cannot create list $($List.name)"
        Log-Message -file $file -Status Error -Message "$($connectionError.Exception.Message)"
        Write-Progress -Activity $Activity -Completed
        break
    }
...
# creating columns
$FileColumn = "Status", "Share", "Team", "Channel", "Folder"
$HomedirColumn = "Status", "Homedir", "Folder"
$ErrorColumn = "Status","Source", "Destination", "Error"

$siteId = ($sites | Where-Object {$_.Displayname -eq $Sharepoint.TeamsName}).id
forEach ($List in $lists){
    start-sleep -Milliseconds 200
    # switch ($List.Name){
    #     "Fileshare Migrations" { $Columns = $FileColumn}
    #     "HomeFolder Migrations" { $Columns = $HomedirColumn}
    #     "Errorlist" {$Columns = $errorColumn}
    # }
    Log-Message -file $file -Status INFO -Message "Creating columns in $($list.Name)"
    Try {
        $create = Create-ListColumns -SiteId $siteId -ListId $list.id -List $List.name
    }
}

# removing first column
ForEach ($list in $lists){
    $apiUri = "https://graph.microsoft.com/v1.0/sites/$siteid/lists/$($list.id)/columns"
    # $apiUri = "https://graph.microsoft.com/v1.0/sites/$siteid/lists/$($list.id)/contentTypes"

    $Columns = RunQueryandEnumerateResults

    $Columns = $columns | Where-Object {$_.DisplayName -eq "Title"}
    ForEach ($Column in $columns){
    $DelUri = $apiUri + "/$($Column.id)"
    ...
    invoke-restmethod -Headers @{Authorization = "Bearer $($Token.access_token)" } -Uri $DelUri -Method PATCH -Body $body
    Invoke-RestMethod -Headers @{Authorization = "Bearer $($Token.access_token)" } -Uri $DelUri -Method DELETE
    }
}
