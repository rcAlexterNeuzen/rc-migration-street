...
if ($Customer.Teams) {
    ...
    ForEach ($upload in $FilesToUpload) {
        if ($Upload.name -like "WF*.zip") {
            $folderlocation = "fileshare%20migrations/SPMT%20logging/$(Get-Date -Format "dd-MM-yyyy")"
        }
        else {
            $folderlocation = "fileshare%20migrations/custom%20logging/$(Get-Date -Format "dd-MM-yyyy")"
        }
